...
# -*- coding: utf-8 -*-
"""
Created on Tue May 31 09:51:00 2022

@author: Karthekeyan Periasamy

Class structure added for the use of the function in the AUTOPC framework
"""
...
class FloatConverter():
    ...
    def bit2float(self,b, num_e_bits=8, num_m_bits=23, bias=127.):
    ...
    def float2bit(self,f, num_e_bits=8, num_m_bits=23, bias=127., dtype=torch.float32):
      ...
      ## MANTISSA
      m1 = self.integer2bit(f - f % 1, num_bits=num_e_bits)
      m2 = self.remainder2bit(f % 1, num_bits=bias)
    ...
    def main(self):
        line="0:01101111:0011010"
        line=line.replace(":", "")
        print(line)
        result=self.bit2float(torch.Tensor([[np.int(f) for f in list(line.strip())]]), num_e_bits=8, num_m_bits=7, bias=127.)
        print(result)

        # test the generation of floating point 1 using a given number of exponent bits and mantissa bits
        # convert the one to floating point
        self.float2bit(torch.tensor(1.0), num_e_bits=8, num_m_bits=23, bias=127)
