...
Edited on May 22, 2023, 20:38
"""

from SWOptimisers.PCOptimiser import PCOptimiser
from Operations.ProductNode.ProductNode import ProductNode
from Operations.SumNode.SumNode import SumNode

class PCSystemTreeOptimiser(PCOptimiser):
    ...
    def __init__(self):
        ...
        # property to know the pipelineDepth of the PCSystemSumNode and PCSystemProductNode
        self.pcSystemAdderPipelineDepth = 0
        self.pcSystemMultiplierPipelineDepth = 0
    ...
    def configure(self):
        ...
        if "AdderPipelineDepth" in self.configurationDetails:
            self.pcSystemAdderPipelineDepth = self.configurationDetails["AdderPipelineDepth"]
        
        # set the pcSystem Multiplier Pipeline Depth
        if "MultiplierPipelineDepth" in self.configurationDetails:
            self.pcSystemMultiplierPipelineDepth = self.configurationDetails["MultiplierPipelineDepth"]
    ...
    def processArithmeticNodes(self):
        ...
        if (len(leftSideLongestPathFromPCNodeList) > len(rightSideLongestPathFromPCNodeList)):
            self.delayOnTheLongestPathOfPC = (((self.pcSystemMultiplierPipelineDepth*leftSideNumberOfProductNodes) + (self.pcSystemAdderPipelineDepth*leftSideNumberOfSumNodes))) + self.pcSystemAdderPipelineDepth*leftSideNumberOfSumNodes
            #print("The current node right side is the longest and the delays on the longest path is", currentNode.delayOnLongestPath)
        else:
            self.delayOnTheLongestPathOfPC = ((self.pcSystemMultiplierPipelineDepth*rightSideNumberOfProductNodes) + (self.pcSystemAdderPipelineDepth*rightSideNumberOfSumNodes)) + self.pcSystemAdderPipelineDepth*leftSideNumberOfSumNodes
                ...
                if ((len(currentNode.input1.split("_")) != 3) or (len(currentNode.input2.split("_")) !=3)):
                    ...
                    if (len(currentNode.input1.split("_")) == 3):
                        ...
                        if (self.heightOfPC-depthOfCurrentNode) != 1:
                            ...
                            shiftRegisterDepth = ((self.pcSystemMultiplierPipelineDepth*numberOfProductNodes) + (self.pcSystemAdderPipelineDepth*numberOfSumNodes)-1)
